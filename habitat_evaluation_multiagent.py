...
def _absolutize_habitat_paths(cfg: DictConfig) -> None:
    """Resolve dataset and scene paths against the Lite project root."""
    def _to_project_path(path_value):
        if not isinstance(path_value, str) or not path_value:
            return path_value
        path = Path(path_value).expanduser()
        return str(path if path.is_absolute() else (PROJECT_ROOT / path).resolve(strict=False))

    with habitat.config.read_write(cfg):
        if "data_path" in cfg.habitat.dataset:
            cfg.habitat.dataset.data_path = _to_project_path(cfg.habitat.dataset.data_path)
        for key in ("scenes_dir", "scene_dataset"):
            ...
            if key in cfg.habitat.simulator:
                cfg.habitat.simulator[key] = _to_project_path(cfg.habitat.simulator[key])
        if "scene" in cfg.habitat.simulator:
            cfg.habitat.simulator.scene = _to_project_path(cfg.habitat.simulator.scene)
...
def main(cfg: DictConfig) -> None:
    ...
    if flag_once and flag_batch:
        raise ValueError("Use either test_epi_num or test_episode_count, not both.")
    ...
    episode_run_count = (
        1 if flag_once else min(test_episode_count, number_of_episodes)
        if flag_batch else number_of_episodes - num_total
    )
        ...
        while not rospy.is_shutdown():
            ...
            if multi_agent:
                ...
                for agent_name in perception_agent_names:
                    ...
                    itm_score_pub_name = f"/clip/{agent_name}/cosine_score"
